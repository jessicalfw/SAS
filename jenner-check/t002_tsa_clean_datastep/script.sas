/* TSA Project - Data Cleaning stage (jenner-check bundle) */
/* Adapted from TSA.sas: original read "&path/TSAClaims2002_2017.csv" via a  */
/* PROC IMPORT off a libname pointed at ~/ECRB94/data on the author's        */
/* machine. The jenner-check runner uploads script text only (no data       */
/* files), so the import is replaced with an inline pipe-delimited          */
/* DATALINES block shaped like the same 16-column TSA claims layout their   */
/* PROC IMPORT produces; the dedup/sort/DATA-step cleaning logic below       */
/* (Claim_Site, Disposition, Claim_Type normalization, date-issue flagging, */
/* labels/formats) is copied unchanged.                                     */
